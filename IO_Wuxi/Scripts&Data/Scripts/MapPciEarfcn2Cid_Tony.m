...
pciEarfcn=cellInfo(:,CELL_PHYSICAL_CELLID)+cellInfo(:,CELL_EARFCNDL)*512;
msr_sc=measurement(:,SC_COL);
msr_nb=measurement(:,NB_COL);
msr_nb_ss=measurement(:,SS_COL(1,2:7));
msr_loc=measurement(:,[LAT_COL LONG_COL]);
%%
GPSIndex = find(msr_loc(:,1)~=-1);
if isempty(GPSIndex)
    disp(['Indoor Mesurements without GPS']);
elseif length(GPSIndex)<size(msr_loc,1)
    disp(['Included Invalid GPS']);
else
    %%  
    cellID=cellInfo(:,CELL_ID);
    earfcn=cellInfo(:,CELL_EARFCNDL);

    ref=mean(cellInfo(:,[CELL_LAT CELL_LONG]));
    cellET=convertlatlong2enu(cellInfo(:,[CELL_LAT CELL_LONG])*pi/180, ref*pi/180);
        ...
        for j=1:id_num
            ...
            if isnan(id) || id==-1
                ...
                elseif (sum(tt)==0)
                    ...
                    notFound=notFound+1;
                    sc_error(i,j)=id;
                elseif (sum(tt)>1)&(msr_ET(i,1)~=-1*pi/180)&(msr_ET(i,2)~=-1*pi/180)
                    %celET=cellET(tt,:);
                    %cel=cellInfo(tt,:);
                    celId=cellID(tt,:);
                    celET=cellET(tt,:);
                    if (msr_ET(i,1)==-1*pi/180)|(msr_ET(i,2)==-1*pi/180)
                        pause;
                        Tag = input('GPS[Lat,Long] Tag of  Known Data = ');
                        Tag
                        msr_ET(i,:)=Tag*pi/180;
                    end
