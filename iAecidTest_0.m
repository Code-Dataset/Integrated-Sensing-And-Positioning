...
% chu: [error Ellipse SimilarMeasurements numAllMeasurements]=iAecidTest_0(test_req, new_test_dataset, 'rscp', celldata_LTE_new,'')
...
function [error Ellipse SimilarMeasurements numAllMeasurements]=iAecidTest_0(positioningRequest, measurements, criteria, cellInfo, googlemap)
ImportedDataFormat;
...
% for i=1:50
    if nargin == 5
        [Ellipse(i,:) conf S N errorCode]=iAecidMain_0(positioningRequest(i,:), measurements, criteria, cellInfo, googlemap);
    else
        [Ellipse(i,:) conf S N errorCode]=iAecidMain_0(positioningRequest(i,:), measurements, criteria, cellInfo);
